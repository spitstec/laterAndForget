...
tXt txtDelete(tXt tx,int bpos,int len) {
  ...
  if (bpos < 0)
    bpos = limii(ln+bpos-1,0,ln-1) ;
  if (bpos > 0) {
    memcpy((char *)rslt,tx,bpos) ;
    txtpoolidx+= bpos ; }
  if (bpos+len < ln) {
    memcpy((char *)&rslt[bpos],&tx[bpos+len],ln-len-bpos) ;
...
tXt txtMalloc(int len) {
  ...
  tXt rslt = &txtpool[txtpoolidx] ;
  if (len > txtMAXLEN) {
    snprintf(txtErrorBuf,txtMAXLEN,"txtmalloc overflow (len=%d)",len) ;
    len = txtMAXLEN ; }
  txtpoolidx += len ;
  txtFixpool() ;
  ZeroMemory((void *)rslt,len) ;
  return rslt ; }
...
// reverse a string
tXt txtFlip(tXt s) {
  tXt rslt = (char *) txtConcat(s,NULL) ;
  char * p = (char *) &rslt[strlen(s)-1] ;
  while (*s)
    *p-- = *s++ ; 
  return rslt ; }

//put char in const string
#define txtPoke(str,index,ch) ((char *)(str))[index] = ch
...
tXt txtFill(char c,int len) {
  ...
  char * s = (char *) txtMalloc(len+1) ;
  s[len--] = 0 ;
  while (len >= 0)  {
    s[len--] = c ; }
  return s ; }

//fill a string with repeated strings
tXt txtRepeat(tXt s , int n) {
  if (n < 0)
    n = 0 ;
  int li = strlen(s) ;
  if (n*li > txtMAXLEN-1)
    n = (txtMAXLEN-1)/li ;
  tXt rslt = (char *)txtMalloc(n*li) ;
  char * p = (char *)rslt ;
  if (n>0)
    while (n--) {
      memcpy((char *)p,s,li) ;
